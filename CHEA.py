...
# --------------------------- Step 2: Filter DEGs --------------------------- #
# Filter DEGs with FDR < 0.1
filtered_deg = df[df['FDR'] < 0.05].copy()
...
if not final_df.empty:
    ...
    print(f"Identified {len(final_df)} TF-target relationships with target FDR < 0.05")
else:
...
for node, data in G.nodes(data=True):
    ...
    # Scale size by log2FC magnitude
    size = abs(log2FC) * 10 + 10
    net.add_node(node, title=f"log2FC: {log2FC:.2f}", color=color, size=size)
